...
actor class Backend() {
  ...
  public shared (msg) func hasAcceptedTerms(userId : Principal) : async Bool {
  ...
  public shared (msg) func getUserStreakAmount(userId : Principal) : async Nat {
    if (isAdmin(msg.caller) or (userId == msg.caller and not Principal.isAnonymous(msg.caller))) {
  ...
  public shared (msg) func getUserStreakPercentage(userId : Principal) : async Nat {
    if (isAdmin(msg.caller) or (userId == msg.caller and not Principal.isAnonymous(msg.caller))) {
  ...
  public shared (msg) func getUserAllStreak(userId : Principal) : async Types.SerializedUserStreak {
    if (isAdmin(msg.caller) or (userId == msg.caller and not Principal.isAnonymous(msg.caller))) {
  ...
  public shared (msg) func getUserStreakTime(userId : Principal) : async Int {
    if (isAdmin(msg.caller) or (userId == msg.caller and not Principal.isAnonymous(msg.caller))) {
