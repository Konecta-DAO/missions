...
module Helpers {
  ...
  public func findActionInstanceInFlow(flowObj : Json.Json, stepIdToFind : Nat, instanceIdToFind : Nat) : Result.Result<Json.Json, Text> {
    switch (Json.get(flowObj, "steps")) {
      case (?(#array(stepsArray))) {
        for (stepObj in stepsArray.vals()) {
          let stepIdOpt = switch (Json.get(stepObj, "stepId")) {
            case (?#number(#int(i))) {
              if (i >= 0) { ?Int.abs(i) } else { null };
            };
            case _ { null };
          };

          if (stepIdOpt == ?stepIdToFind) {
            // Found the right step, now find the instance
            switch (Json.get(stepObj, "item.#SingleAction")) {
              case (?instanceObj) {
                let instanceIdOpt = switch (Json.get(instanceObj, "instanceId")) {
                  case (?#number(#int(i))) {
                    if (i >= 0) { ?Int.abs(i) } else { null };
                  };
                  case _ { null };
                ...
                if (instanceIdOpt == ?instanceIdToFind) {
                  return #ok(instanceObj); // Found it!
                };
              };
              case _ {}; // Could be an ActionGroup, not supported for this helper
            };
          };
        };
        return #err("Step/Instance combination not found.");
      };
      case _ { return #err("`steps` field is not a valid array.") };
    };
  };

  // A helper to get a Nat array value from a parameter binding.
  public func getNatArrayFromParamBinding(instanceObj : Json.Json, paramName : Text) : ?[Nat] {
    switch (Json.get(instanceObj, "parameterBindings")) {
      case (?#array(bindings)) {
        for (bindingObj in bindings.vals()) {
          let nameOpt = switch (Json.get(bindingObj, "parameterName")) {
            case (?#string(s)) { ?s };
            case _ { null };
          };

          if (nameOpt == ?paramName) {
            // Found the right parameter, now get its literal value
            switch (Json.get(bindingObj, "valueSource.#LiteralValue")) {
                ...
                switch (Json.parse(jsonText)) {
                  case (#ok(#array(arr))) {
                    ...
                    for (item in arr.vals()) {
                      switch (item) {
                        // CORRECTED LINE: Check for non-negative Int and convert to Nat
                        case (#number(#int(i))) {
                          if (i >= 0) {
                            nats := Array.append(nats, [Int.abs(i)]);
                          };
                        };
                        case _ {};
                      };
  ...
  // A helper to get a single Nat value from a parameter binding.
  public func getNatFromParamBinding(instanceObj : Json.Json, paramName : Text) : ?Nat {
    switch (Json.get(instanceObj, "parameterBindings")) {
      case (?#array(bindings)) {
        for (bindingObj in bindings.vals()) {
          let nameOpt = switch (Json.get(bindingObj, "parameterName")) {
          ...
          if (nameOpt == ?paramName) {
            switch (Json.get(bindingObj, "valueSource.#LiteralValue")) {
              case (?#string(jsonText)) {
                switch (Json.parse(jsonText)) {
                  // CORRECTED LINE: Check for non-negative Int and convert to Nat
                  case (#ok(#number(#int(i)))) {
                    if (i >= 0) { return ?Int.abs(i) } else { return null };
                  };
                  case _ { return null };
                };
              };
