...
module JsonUtils {
  ...
  public func serializeActionReturnedDataToJsonObj(data : Types.ActionReturnedData) : Result.Result<Json.Json, Text> {
    switch (data) {
      ...
      case (#NftOwnershipResult(res)) {
        var fields : [(Text, Json.Json)] = [
          ("nftCanisterId", Json.str(Principal.toText(res.nftCanisterId))),
          ("principalChecked", Json.str(Principal.toText(res.principalChecked))),
          ("hasRequiredNft", Json.bool(res.hasRequiredNft)),
          ("ownedTokenCount", Json.int(res.ownedTokenCount)),
          ("verificationStatus", serializeActionStatusOutcomeToJson(res.verificationStatus)),
        ];
        switch (res.detailMessage) {
          case (?msg) {
            fields := Array.append(fields, [("detailMessage", Json.str(msg))]);
          };
          case null {};
        };
        return #ok(Json.obj(fields));
      ...
      case (#LeaderboardResult(res)) {
        var fields : [(Text, Json.Json)] = [];

        let statusText = switch (res.status) {
          case (#Pending) "Pending";
          case (#Settled) "Settled";
          case (#Failed) "Failed";
        };
        fields := Array.append(fields, [("status", Json.str(statusText))]);

        switch (res.settlementTime) {
          case (?time) {
            fields := Array.append(fields, [("settlementTime", Json.int(time))]);
          };
          case null {
            fields := Array.append(fields, [("settlementTime", Json.nullable())]);
          };
        };

        switch (res.winners) {
          case (?winnersList) {
            var winnersJson : [Json.Json] = [];
            for (winner in winnersList.vals()) {
              winnersJson := Array.append(
                winnersJson,
                [
                  Json.obj([
                    ("rank", Json.int(winner.rank)),
                    ("userUuid", Json.str(winner.userUuid)),
                    ("totalPoints", Json.int(winner.totalPoints)),
                    ("rewardAmount", Json.int(winner.rewardAmount)),
                  ])
                ],
              );
            };
            fields := Array.append(fields, [("winners", Json.arr(winnersJson))]);
          };
          case null {
            fields := Array.append(fields, [("winners", Json.nullable())]);
          };
        };
      ...
      case (#EventJoinResult(res)) {
        var fields : [(Text, Json.Json)] = [
          ("eventId", Json.str(res.eventId)),
          ("principalChecked", Json.str(Principal.toText(res.principalChecked))),
          ("hasJoined", Json.bool(res.hasJoined)),
          ("verificationStatus", serializeActionStatusOutcomeToJson(res.verificationStatus)),
        ];
        return #ok(Json.obj(fields));
      };
      case (#EventCreateAnyResult(res)) {
        var fields : [(Text, Json.Json)] = [
          ("principalChecked", Json.str(Principal.toText(res.principalChecked))),
          ("hasCreatedEvents", Json.bool(res.hasCreatedEvents)),
          ("createdEventsCount", Json.int(res.createdEventsCount)),
          ("verificationStatus", serializeActionStatusOutcomeToJson(res.verificationStatus)),
        ];
        return #ok(Json.obj(fields));
      };
